import { Event } from "./types";

export const FALLBACK_EVENTS: Event[] = [
  {
    id: "fb-1",
    title: "Google Solution Challenge 2026",
    organization: "Google Developers",
    type: "hackathon",
    description: "Build a solution for one or more of the United Nations 17 Sustainable Development Goals using Google technology.",
    location: "Online",
    date: "January - March 2026",
    link: "https://developers.google.com/community/challenge",
    applyLink: "https://developers.google.com/community/challenge",
    price: "Free",
    isPaid: false,
    coordinates: { lat: 37.422488, lng: -122.084248 }
  },
  {
    id: "fb-2",
    title: "Smart India Hackathon (SIH) 2026",
    organization: "Ministry of Education, India",
    type: "hackathon",
    description: "World's biggest open innovation model that provides students with a platform to solve some of the pressing problems we face in our daily lives.",
    location: "Pan India / Online",
    date: "August - December 2026",
    link: "https://www.sih.gov.in/",
    applyLink: "https://www.sih.gov.in/",
    price: "Free",
    isPaid: false,
    coordinates: { lat: 28.6139, lng: 77.2090 }
  },
  {
    id: "fb-3",
    title: "Microsoft Imagine Cup 2026",
    organization: "Microsoft",
    type: "hackathon",
    description: "The Imagine Cup is a global competition that empowers the next generation of computer science students to team up and use their creativity to solve problems.",
    location: "Online / Global",
    date: "October 2025 - May 2026",
    link: "https://imaginecup.microsoft.com/",
    applyLink: "https://imaginecup.microsoft.com/",
    price: "Free",
    isPaid: false,
    coordinates: { lat: 47.6740, lng: -122.1215 }
  },
  {
    id: "fb-4",
    title: "PM Internship Scheme 2026",
    organization: "Government of India",
    type: "scheme",
    description: "A flagship scheme providing internship opportunities in top 500 companies to 1 crore youth over 5 years.",
    location: "India",
    date: "Ongoing",
    link: "https://pminternship.mca.gov.in/",
    applyLink: "https://pminternship.mca.gov.in/",
    price: "Free",
    isPaid: false,
    coordinates: { lat: 28.6139, lng: 77.2090 }
  },
  {
    id: "fb-5",
    title: "AWS Cloud Up for Her",
    organization: "Amazon Web Services",
    type: "program",
    description: "A flexible, community-based learning program for women to prepare for AWS Cloud Practitioner certification.",
    location: "Online",
    date: "Rolling Basis",
    link: "https://aws.amazon.com/training/digital/aws-cloud-up-for-her/",
    applyLink: "https://aws.amazon.com/training/digital/aws-cloud-up-for-her/",
    price: "Free",
    isPaid: false,
    coordinates: { lat: 47.6062, lng: -122.3321 }
  },
  {
    id: "fb-6",
    title: "TCS CodeVita Season 13",
    organization: "Tata Consultancy Services",
    type: "hackathon",
    description: "The world's largest competitive programming contest for students and recent graduates.",
    location: "Online",
    date: "November 2025 - March 2026",
    link: "https://www.tcscodevita.com/",
    applyLink: "https://www.tcscodevita.com/",
    price: "Free",
    isPaid: false,
    coordinates: { lat: 19.0760, lng: 72.8777 }
  },
  {
    id: "fb-7",
    title: "Meta Hacker Cup 2026",
    organization: "Meta",
    type: "hackathon",
    description: "Meta's annual open programming competition that tests your problem-solving skills with algorithmic challenges.",
    location: "Online",
    date: "September - December 2026",
    link: "https://www.facebook.com/codingcompetitions/hacker-cup",
    applyLink: "https://www.facebook.com/codingcompetitions/hacker-cup",
    price: "Free",
    isPaid: false,
    coordinates: { lat: 37.4848, lng: -122.1484 }
  },
  {
    id: "fb-8",
    title: "Post-Matric Scholarship Scheme",
    organization: "Ministry of Minority Affairs",
    type: "scheme",
    description: "Scholarship for students belonging to minority communities to pursue higher education from Class 11 to Ph.D.",
    location: "India",
    date: "July - October 2026",
    link: "https://scholarships.gov.in/",
    applyLink: "https://scholarships.gov.in/",
    price: "Free",
    isPaid: false,
    coordinates: { lat: 28.6139, lng: 77.2090 }
  },
  {
    id: "fb-9",
    title: "GitHub Octernships",
    organization: "GitHub",
    type: "program",
    description: "Connects students with industry partners for paid remote internships to work on real-world open source projects.",
    location: "Remote",
    date: "Quarterly Batches",
    link: "https://github.com/education/Octernships",
    applyLink: "https://github.com/education/Octernships",
    price: "Paid (Stipend)",
    isPaid: false,
    coordinates: { lat: 37.7749, lng: -122.4194 }
  },
  {
    id: "fb-10",
    title: "MLH Fellowship",
    organization: "Major League Hacking",
    type: "program",
    description: "A 12-week internship alternative for aspiring software engineers to contribute to open source projects.",
    location: "Remote",
    date: "Spring, Summer, Fall Batches",
    link: "https://fellowship.mlh.io/",
    applyLink: "https://fellowship.mlh.io/",
    price: "Paid (Stipend)",
    isPaid: false,
    coordinates: { lat: 40.7128, lng: -74.0060 }
  },
  {
    id: "fb-11",
    title: "Flipkart GRiD 6.0",
    organization: "Flipkart",
    type: "hackathon",
    description: "Flipkart's flagship engineering campus challenge to solve real-world e-commerce problems.",
    location: "Online / India",
    date: "July - September 2026",
    link: "https://unstop.com/hackathons/flipkart-grid-60-software-development-track-flipkart-894234",
    applyLink: "https://unstop.com/hackathons/flipkart-grid-60-software-development-track-flipkart-894234",
    price: "Free",
    isPaid: false,
    coordinates: { lat: 12.9716, lng: 77.5946 }
  },
  {
    id: "fb-12",
    title: "Google Summer of Code (GSoC) 2026",
    organization: "Google",
    type: "program",
    description: "A global, online program focused on bringing new contributors into open source software development.",
    location: "Online",
    date: "January - August 2026",
    link: "https://summerofcode.withgoogle.com/",
    applyLink: "https://summerofcode.withgoogle.com/",
    price: "Paid (Stipend)",
    isPaid: false,
    coordinates: { lat: 37.4224, lng: -122.0842 }
  }
];
