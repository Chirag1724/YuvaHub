...
const EmployerAnalyticsDashboard = lazy(() => import('./pages/EmployerAnalyticsDashboard'));
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 flex flex-col font-sans">
        ...
        <header className="sticky top-0 z-50 h-[60px] bg-surface dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between px-6 lg:px-12">
          ...
          <nav className="hidden md:flex items-center gap-Here is the complete, updated code with the merge conflict fully resolved. You can copy and paste this entire block to replace your current file. Both the `Layout` and `Mail` icons from the conflicting branches have been combined and preserved.
...
function App() {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'leaderboard': return <Leaderboard/>;
      case 'ai_assistant': return (
        <Suspense <div className="flex flex-col items-center justify-center h-full min-h-[400px] gap-4" fallback="{">
            <div className="flex gap-2">
              <div className="w-2.5 h-2.5 rounded-full bg-[#2563EB] animate-bounce" style={{ animationDelay: '0ms' }}></div>
              <div className="w-2.5 h-2.5 rounded-full bg-[#2563EB] animate-bounce" style={{ animationDelay: '150ms' }}></div>
              <div className="w-2.5 h-2.5 rounded-full bg-[#2563EB] animate-bounce" style={{ animationDelay: '300ms' }}></div>
      ...
      case 'support': return <Support/>;
      case 'about': return <AboutTab/>;
      case 'help': return gettingStartedStep ? <GettingStartedDetail any} as stepId="{gettingStartedStep"/> : <HelpCenterPage/>;
      case 'mock_interview': return <MockInterviewRoom/>;
      case 'research_grant_telemetry':
      case 'grant_telemetry': return <div>Telemetry Lab (WIP)</div>;
      case 'watchlist_manager': return <WatchlistManager/>;
  ...
  if (user && profile && !hasOnboarded) {
    return (
      <Suspense fallback="{<LoadingScreen" fullScreen="{true}"/>}>
        <OnboardingFlow onComplete="{(updated)" profile="{profile}" user="{user}"> {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2.5 truncate">
                      <Icon ${isActive 'text-primary-blue 'text-text-muted : ? aria-hidden="true" className="{`w-4" dark:text-blue-400' dark:text-gray-500'}`} h-4 shrink-0/>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <Icon ${isActive 'text-primary-blue' 'text-text-muted'}`} : ? aria-hidden="true" className="{`w-4" h-4 shrink-0/>
                        <span>{tab.label}</span>
                      </div>
                      {tab.badge && (
