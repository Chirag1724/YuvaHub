...
if (serviceAccountKeyRaw) {
  try {
    ...
    logger.info(`[PushWorker] FCM Credentials configured for project: ${projectId}`);
  } catch (err: any) {
    logger.error({ err, projectId }, "Failed to parse FCM credentials");
  }
}
...
export const pushWorker = new Worker<PushJobData>(
  ...
  async (job: Job<PushJobData>) => {
    ...
    logger.info(`[PushWorker] Processing job ${job.id} for userId ${userId}`);
    ...
    if (job.data && userId !== "global-subscribers") {
      ...
      logger.info(`[PushWorker] Fetching FCM token for user: ${userId}`);
    }
    ...
    if (hasFcmCredentials && fcmToken) {
      try {
        ...
        logger.info(`[PushWorker] Successfully delivered FCM push to ${userId}`);
      } catch (err: any) {
        logger.error({ err, userId }, "Real FCM push failed");
      ...
      logger.info(`[PushWorker] Simulated Push Sent. To: ${userId} | Message: ${message}`);
    }
  },
...
pushWorker.on("completed", (job) => {
  logger.info(`[PushWorker] Job ${job.id} completed successfully`);
});
...
pushWorker.on("error", (err) => {
  if (!pushWorkerErrorLogged) {
    logger.warn('[PushWorker] Redis connection offline. Worker listening paused.');
