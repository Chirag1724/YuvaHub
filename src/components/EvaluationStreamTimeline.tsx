import React from 'react';
import { Activity, ShieldCheck, CheckCircle2, Gavel } from 'lucide-react';

const RECENT_EVALUATION_LOGS = [
  {
    id: 'eval-log-1',
    projectName: 'NeuroShield',
    action: 'Jury Score Submitted (96.2 / 100)',
    status: 'EVALUATION_COMPLETED',
    timestampAgo: '8 mins ago',
  },
  {
    id: 'eval-log-2',
    projectName: 'ChainPay ZK',
    action: 'GitHub Code Audit AST Analysis Passed',
    status: 'AST_CLEAN',
    timestampAgo: '25 mins ago',
  },
  {
    id: 'eval-log-3',
    projectName: 'AgriSense IoT',
    action: 'Shortlisted for Grand Finale Pitch Round',
    status: 'FINALE_QUALIFIED',
    timestampAgo: '50 mins ago',
  },
];

export default function EvaluationStreamTimeline() {
  return (
    <div className="bg-slate-900/60 border border-slate-800/80 rounded-3xl p-6 md:p-8 backdrop-blur-md">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h3 className="text-xl font-bold text-white flex items-center gap-2">
            <Activity className="w-5 h-5 text-amber-400" /> Live Jury Score & AST Plagiarism Stream
          </h3>
          <p className="text-slate-400 text-xs mt-1">Real-time jury panel submissions, GitHub repository static code quality parsing, and leaderboard recalculations.</p>
        </div>

        <div className="flex items-center gap-2 bg-slate-950 px-3 py-1.5 rounded-xl border border-slate-800 text-xs text-amber-300 font-semibold font-mono">
          <Gavel className="w-4 h-4 text-amber-400" /> Jury Panel Active
        </div>
      </div>

      <div className="space-y-4">
        {RECENT_EVALUATION_LOGS.map((log) => (
          <div
            key={log.id}
            className="bg-slate-950/90 border border-slate-800/90 rounded-2xl p-5 hover:border-amber-500/30 transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-4"
          >
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="bg-amber-500/10 text-amber-400 text-[11px] font-mono px-2 py-0.5 rounded border border-amber-500/20 font-bold">
                  {log.status}
                </span>
                <span className="text-slate-500 text-xs font-mono">{log.timestampAgo}</span>
              </div>
              <h4 className="text-base font-bold text-slate-100">{log.action}</h4>
              <div className="text-xs text-slate-400 mt-1 font-mono">
                Project: <span className="text-slate-200">{log.projectName}</span>
              </div>
            </div>

            <div className="flex items-center gap-3">
              <div className="text-xs text-emerald-400 flex items-center gap-1 font-semibold">
                <CheckCircle2 className="w-4 h-4" /> Verified
              </div>
            </div>
          </div>
        ))}
      </div>
    </div>
  );
}
