...
export const CardioAdmissionModal: React.FC<CardioAdmissionModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
      <div className="bg-primary-blue border border-border-theme rounded-3xl w-full max-w-4xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        {/* Header */}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 overflow-y-auto max-h-[80vh]">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div>
                <label className="block text-xs text-text-muted mb-1">Bed Number</label>
                <input
                  type="text"
                  value={bedNumber}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div>
                <label className="block text-xs text-text-muted mb-1">Age (Years)</label>
                <input
                  type="number"
                  value={age}
                  onChange={(e) => setAge(Number(e.target.value))}
                  className="w-full bg-slate-950 border border-border-theme rounded-xl px-3 py-2 text-xs text-white font-mono"
                  required
                />
              </div>
              <div>
          ...
          <div className="space-y-3 pt-3 border-t border-border-theme">
            ...
            <div>
              <label className="block text-xs text-text-muted mb-1">Primary Clinical Diagnosis</label>
              <input
                type="text"
                value={diagnosis}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                <label className="block text-xs text-text-muted mb-1">Shock Etiology</label>
                <select
                  value={shockEtiology}
              ...
              <div>
                <label className="block text-xs text-text-muted mb-1">SCAI Shock Stage</label>
                <select
                  value={scaiStage}
              ...
              <div>
                <label className="block text-xs text-text-muted mb-1">MCS Support Device</label>
                <select
                  value={mcsDevice}
          ...
          <div className="space-y-3 pt-3 border-t border-border-theme">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 font-mono">
              <div>
                <label className="block text-xs text-text-muted mb-1">Heart Rate (bpm)</label>
                <input
                  type="number"
                  value={hr}
              ...
              <div>
                <label className="block text-xs text-text-muted mb-1">SBP (mmHg)</label>
                <input
                  type="number"
                  value={sbp}
              ...
              <div>
                <label className="block text-xs text-text-muted mb-1">DBP (mmHg)</label>
                <input
                  type="number"
                  value={dbp}
              ...
              <div>
                <label className="block text-xs text-text-muted mb-1">CVP (mmHg)</label>
                <input
                  type="number"
                  value={cvp}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 font-mono">
              <div>
                <label className="block text-xs text-text-muted mb-1">PAS (mmHg)</label>
                <input
                  type="number"
                  value={pas}
              ...
              <div>
                <label className="block text-xs text-text-muted mb-1">PAD (mmHg)</label>
                <input
                  type="number"
                  value={pad}
              ...
              <div>
                <label className="block text-xs text-text-muted mb-1">PCWP / Wedge (mmHg)</label>
                <input
                  type="number"
                  value={pcwp}
              ...
              <div>
                <label className="block text-xs text-text-muted mb-1">Cardiac Output (L/min)</label>
                <input
                  type="number"
                  step="0.1"
                  value={co}
            ...
            <div className="bg-slate-950 border border-border-theme rounded-xl p-3 grid grid-cols-2 sm:grid-cols-4 gap-2 text-[11px] font-mono">
              <div>
                <span className="text-text-muted">MAP:</span> <strong className="text-cyan-300">{map} mmHg</strong>
              </div>
              <div>
                <span className="text-text-muted">CI:</span> <strong className="text-cyan-300">{ci} L/min/m²</strong>
              </div>
              <div>
                <span className="text-text-muted">CPO:</span> <strong className={cpo < 0.6 ? "text-red-400 font-black" : "text-emerald-400"}>{cpo} Watts</strong>
              </div>
              <div>
                <span className="text-text-muted">PAPi:</span> <strong className={papi < 0.9 ? "text-amber-400 font-bold" : "text-slate-200"}>{papi}</strong>
              </div>
            </div>
          </div>
