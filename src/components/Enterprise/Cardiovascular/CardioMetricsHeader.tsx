...
export const CardioMetricsHeader: React.FC<CardioMetricsHeaderProps> = ({
  ...
  return (
    <div className="bg-primary-blue border border-border-theme rounded-2xl p-6 shadow-2xl relative overflow-hidden mb-6">
      {/* Background ambient glow */}
      <div className="absolute -top-24 -right-24 w-96 h-96 bg-rose-500/10 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute -bottom-24 -left-24 w-96 h-96 bg-cyan-500/200/10 rounded-full blur-3xl pointer-events-none" />

      {/* Top Bar: Title, Command Badge & Quick Actions */}
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 pb-6 border-b border-border-theme">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 rounded-xl font-semibold text-sm transition-all shadow-md ${
              isLiveStreaming
                ? "bg-emerald-500/200/20 text-emerald-300 border border-emerald-500/40 hover:bg-emerald-500/200/30"
                : "bg-surface-secondary text-slate-300 border border-border-theme hover:bg-slate-700"
            }`}
          >
            <span
              className={`w-2.5 h-2.5 rounded-full ${
                isLiveStreaming ? "bg-emerald-400 animate-pulse" : "bg-surface0"
              }`}
            />
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl font-semibold text-sm bg-surface-secondary hover:bg-slate-700 text-cyan-300 border border-cyan-500/30 transition-all shadow-md"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 mt-6">
        {/* Census */}
        <div className="bg-slate-950/70 border border-border-theme/80 rounded-xl p-4 flex flex-col justify-between">
          ...
          <div className="mt-2 flex items-baseline gap-2">
            ...
            <span className="text-xs text-text-muted">/ {metrics.totalAvailableBeds} Beds</span>
          </div>
        ...
        <div className="bg-slate-950/70 border border-border-theme/80 rounded-xl p-4 flex flex-col justify-between">
          ...
          <div className="mt-2 flex items-baseline gap-2">
            ...
            <span className="text-xs text-text-muted">Pts at risk</span>
          </div>
        ...
        <div className="bg-slate-950/70 border border-border-theme/80 rounded-xl p-4 flex flex-col justify-between">
          ...
          <div className="mt-2 flex items-baseline gap-2">
            ...
            <span className="text-xs text-text-muted">Membrane Clotting</span>
          </div>
        ...
        <div className="bg-slate-950/70 border border-border-theme/80 rounded-xl p-4 flex flex-col justify-between">
          ...
          <div className="mt-2 flex items-baseline gap-2">
            ...
            <span className="text-xs text-text-muted">Δ SpO2 &gt; 10%</span>
          </div>
      ...
      <div className="mt-5 pt-4 border-t border-border-theme/80 flex flex-wrap items-center justify-between gap-4 text-xs">
        <div className="flex items-center gap-2 text-text-muted font-semibold uppercase tracking-wider">
          <Flame className="w-4 h-4 text-amber-400" />
          <span>SCAI Shock Stratification:</span>
        </div>
        <div className="flex flex-wrap items-center gap-2 sm:gap-4">
          <span className="px-2.5 py-1 rounded-lg bg-emerald-500/200/10 border border-emerald-500/30 text-emerald-300 font-mono">
            Stage A (At Risk): <strong className="text-white">{metrics.scaiStageDistribution.stageA}</strong>
          </span>
          <span className="px-2.5 py-1 rounded-lg bg-blue-500/200/10 border border-blue-500/30 text-blue-300 font-mono">
            Stage B (Beginning): <strong className="text-white">{metrics.scaiStageDistribution.stageB}</strong>
          </span>
          <span className="px-2.5 py-1 rounded-lg bg-amber-500/200/10 border border-amber-500/30 text-amber-300 font-mono">
